...
import { Phone, ArrowDown, Play, Pause, Volume2, VolumeX, Maximize2, Minimize2 } from 'lucide-react';
...
const MobileFrameAnimation: React.FC = () => {
  return (
    <div 
      ...
      style={{
        backgroundImage: `url(/animation/animation.webp)`,
...
export const MobileHero: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (videoRef.current) {
      ...
      console.log('MobileHero: Громкость установлена:', videoRef.current.volume);
    } else {
  ...
  const handleMute = () => {
    ...
    videoRef.current.volume = 0.5;
    console.log('MobileHero: Громкость после переключения mute:', videoRef.current.volume);
    setIsMuted(!isMuted);
  };
      ...
      if (wasFullscreen && !isCurrentlyFullscreen && videoRef.current) {
        if (videoRef.current.paused) {
          videoRef.current.play().catch(err => {
            console.log('Не удалось автоматически возобновить воспроизведение:', err);
          });
          setIsPlaying(true);
        }
      }
    ...
    >
      ...
      <div className="container mx-auto px-4 py-20 relative z-10 flex flex-col items-center">
        ...
        <motion.div
          ...
          animate={loadingDone ? { scale: 1, opacity: 1 } : { scale: 0.8, opacity: 0 }}
          ...
          >
            ...
            <video
              ...
              onLoadedMetadata={(e) => {
                ...
                video.volume = 0.5;
                console.log('MobileHero: Громкость при загрузке метаданных:', video.volume);
              }}
            >
        ...
        >
          ...
          <motion.div variants={blockVariants} className="space-y-6">
            <div className="flex items-center justify-center gap-4 mb-6" style={{ marginLeft: '-20px' }}>
              <motion.div
              ...
              <h1 className="font-furore leading-tight uppercase text-accent text-center" style={{ fontSize: 'clamp(2.4375rem, 5vw, 2.925rem)' }}>
                ПОЧЕМУ<br />ВЫБИРАЮТ<br />НАС?
              </h1>
            </div>
          ...
          <motion.div variants={blockVariants} className="space-y-6">
            <ul className="space-y-4 max-w-lg mx-auto">
              <li className="flex items-center gap-4 text-left">
                <span className="icon-container">
                  <Image 
                    ...
                    width={32} 
                    height={32} 
                    className={`icon-shield${activeBounceIndex === 0 ? ' bounce-check' : ''}`}
                  />
                </span>
              ...
              <li className="flex items-center gap-4 text-left">
                <span className="icon-container">
                  <Image 
                    src="/images/icons/alarm-clock.png" 
                    alt="Пунктуальность" 
                    width={32} 
                    height={32} 
                    className={`icon-alarm${activeBounceIndex === 1 ? ' bounce-check' : ''}`}
                  />
                </span>
                <span className="font-furore text-sm leading-snug text-white uppercase">
              ...
              <li className="flex items-center gap-4 text-left">
                <span className="icon-container">
                  <Image 
                    src="/images/icons/molotok.png" 
                    alt="Инструмент" 
                    width={32} 
                    height={32} 
                    className={`icon-hammer${activeBounceIndex === 2 ? ' bounce-check' : ''}`}
                  />
                </span>
                <span className="font-furore text-sm leading-snug text-white uppercase">
              ...
              <li className="flex items-center gap-4 text-left">
                <span className="icon-container">
                  <Image 
                    ...
                    width={32} 
                    height={32} 
                    className={`icon-ruble${activeBounceIndex === 3 ? ' bounce-check' : ''}`}
                  />
                </span>
                <span className="font-furore text-sm leading-snug text-white uppercase">
          ...
          <motion.div variants={blockVariants} className="space-y-4">
            <div className="flex flex-col gap-3 sm:flex-row">
              ...
              <Button
                ...
                className="h-12 flex-1 rounded-3xl border-accent/60 text-base font-semibold uppercase tracking-wide text-accent hover:bg-accent/10"
              >
                <a href={activeRegion.mobileHref || activeRegion.phoneHref}>Позвонить мастеру</a>
              </Button>
            </div>
